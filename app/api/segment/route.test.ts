...
// Mock modules
const mockRunSAMSegmentation = mock(() => Promise.resolve({
  masks: [
    {
...
function createMockRequest(body: unknown, options?: { headers?: Record<string, string> }): NextRequest {
...
describe('POST /api/segment', () => {
  beforeEach(() => {
    ...
    mockRunSAMSegmentation.mockImplementation(() => Promise.resolve({
      masks: [
        {
          mask: 'base64-encoded-mask-data',
          confidence: 0.95,
    ...
    mockDecodeBase64Image.mockImplementation(() => Buffer.from('fake-image-data'));
  });
  ...
  describe('Valid requests', () => {
    it('should return success response with mask data', async () => {
      ...
      expect(mockDecodeBase64Image).toHaveBeenCalledWith(validRequestBody.image);
  ...
  describe('Invalid JSON', () => {
    it('should return 500 for invalid JSON', async () => {
      ...
      request.json = mock(() => Promise.reject(new SyntaxError('Unexpected end of JSON input')));
  ...
  describe('Schema validation', () => {
    ...
    it('should return 400 for too many points (>20)', async () => {
      const invalidBody = {
        ...validRequestBody,
        points: Array.from({ length: 21 }, (_, i) => ({ x: i * 10, y: i * 10, label: 1 })),
      };
  ...
  describe('Image dimensions validation', () => {
    it('should return 400 when imageWidth exceeds 4096', async () => {
      ...
      expect(response.status).toBe(400);
      expect(data.success).toBe(false);
      expect(data.error).toBe('Image dimensions exceed maximum allowed size (4096x4096)');
