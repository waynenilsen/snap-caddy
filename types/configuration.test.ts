...
import { validateBinConfig, GRIDFINITY_CONSTRAINTS, type GridfinityBinConfig } from './configuration';
...
describe('validateBinConfig', () => {
  ...
  describe('cutoutDepth validation', () => {
    it('should return error when cutoutDepth equals binHeight', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('cutoutDepth (21mm) must be less than binHeight (21mm)');
    });
    ...
    it('should return error when cutoutDepth is greater than binHeight', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('cutoutDepth (25mm) must be less than binHeight (21mm)');
    });
  ...
  describe('wallThickness validation', () => {
    ...
    it('should return warning when wallThickness is below recommended', () => {
      ...
      expect(result.valid).toBe(true);
      expect(result.warnings).toContain('wallThickness below 2mm may result in weak walls');
    });
    ...
    it('should not return warning when wallThickness is at recommended value', () => {
      ...
      expect(result.warnings).not.toContain('wallThickness below 2mm may result in weak walls');
    });
  ...
  describe('multiple errors', () => {
    ...
    it('should return both errors and warnings when applicable', () => {
      ...
      expect(result.errors).toContain('cutoutDepth (20mm) must be less than binHeight (15mm)');
      expect(result.warnings).toContain('binHeight should be in 7mm increments for standard Gridfinity compatibility');
      expect(result.warnings).toContain('wallThickness below 2mm may result in weak walls');
    });
  });
});
