...
test.describe("Capture Step (Step 0)", () => {
  ...
  test("should display the application header with logo and title", async ({
    page,
  }) => {
  ...
  test.skip("should display the step indicator with Capture as first step", async ({
    page,
  }) => {
  ...
  test("should display the Capture Image card with title and description", async ({
    page,
  }) => {
    ...
    const description = page.getByText(
      /Take a photo of your object or upload an existing image/i,
    );
  ...
  test("should display tips section for best results", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Place the object on a contrasting background/i),
    ).toBeVisible();
    await expect(page.getByText(/Ensure good, even lighting/i)).toBeVisible();
    await expect(
      page.getByText(/Include a ruler or known-size object/i),
    ).toBeVisible();
  });
  ...
  test("should enable Next button and navigate to next step after image capture", async ({
    page,
  }) => {
    // Upload an image
    const uploadTab = page.getByRole("tab", { name: /Upload/i });
  ...
  // Note: This test is flaky due to CSS visibility of step indicator spans
  test.skip("should reset wizard when Start Over is clicked", async ({
    page,
  }) => {
    ...
    await page.waitForSelector("text=Click to Select Object", {
      timeout: 5000,
    });
