...
test.describe('Segment Step (Step 1)', () => {
  ...
  test('should display the step indicator with Segment as current step', async ({ page }) => {
  ...
  test('should display the instruction info alert', async ({ page }) => {
    ...
    const infoAlert = page.getByText(/Click on the object you want to extract/i);
  ...
  test('should display the Click to Select Object card', async ({ page }) => {
    ...
    const rightClickHint = page.getByText('Right-click', { exact: true }).first();
    await expect(rightClickHint).toBeVisible();
  });

  test('should display the canvas with the uploaded image', async ({ page }) => {
  ...
  test('should display initial message when no points selected', async ({ page }) => {
    // Check for the "no points" message
    const noPointsMessage = page.getByText(/Click on the object to start segmentation/i);
  ...
  test('should display point counter showing 0 points initially', async ({ page }) => {
  ...
  test('should navigate back to capture step when Back is clicked', async ({ page }) => {
    // Click Back button
    const backButton = page.getByRole('button', { name: 'Go to previous step' });
    await backButton.click();

    // Should go back to Capture step
    await expect(page.getByText('Capture Image', { exact: true })).toBeVisible({ timeout: 5000 });
...
test.describe('Segment Step - Point Interactions', () => {
  ...
  test.skip('should add exclude point on right-click', async ({ page }) => {
    ...
    await canvas.click({ position: { x: box!.width * 0.5, y: box!.height * 0.5 }, button: 'right' });
    await page.waitForTimeout(500);
  ...
  test('should clear all points when Clear All is clicked', async ({ page }) => {
    const canvas = page.locator('canvas');
    const box = await canvas.boundingBox();
    expect(box).not.toBeNull();
...
test.describe('Segment Step - API Integration', () => {
  test('should show loading state when Segment button is clicked', async ({ page }) => {
  ...
  test('should show success message after segmentation completes', async ({ page }) => {
  ...
  test('should show error message when segmentation fails', async ({ page }) => {
    ...
    const errorAlert = page.locator('[role="alert"]').filter({ hasText: /error|unable|failed/i });
  ...
  test('should dismiss error when X button is clicked', async ({ page }) => {
    ...
    const dismissButton = errorAlert.locator('button').filter({ has: page.locator('svg') }).last();
  ...
  test('should navigate to next step after successful segmentation', async ({ page }) => {
    // Mock the segment API
    await page.route('**/api/segment', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    // Wait for success
    await page.waitForSelector('text=Segmentation complete', { timeout: 10000 });
    ...
    const calibrateContent = page.getByText('Select Ruler Reference', { exact: true });
...
test.describe('Segment Step - Accessibility', () => {
  ...
  test('should have proper ARIA labels on control buttons', async ({ page }) => {
    ...
    const segmentButton = page.getByRole('button', { name: /Segment/i });
    await expect(segmentButton).toHaveAttribute('aria-label', 'Generate segmentation');
  });
...
test.describe('Segment Step - Edge Cases', () => {
  ...
  test('should clear mask and return to point selection when Clear All is clicked after segmentation', async ({
    ...
    await expect(page.getByText('0 points')).toBeVisible();
    await expect(page.getByText(/Click on the object to start segmentation/i)).toBeVisible();
  });
});
