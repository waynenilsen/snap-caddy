...
describe('runSAMSegmentation', () => {
  ...
  describe('Successful segmentation', () => {
    ...
    it('should handle multiple points', async () => {
      ...
      const mockFetch = mock(async (url: string, options?: any) => {
        if (options?.method === 'POST') {
          const body = JSON.parse(options.body);
          expect(body.input.point_coords).toEqual([[10, 20], [30, 40], [50, 60]]);
  ...
  describe('Error handling', () => {
    it('should throw error on failed prediction creation', async () => {
      ...
      await expect(runSAMSegmentation(params)).rejects.toThrow('Replicate API error');
    });
    ...
    it('should throw error when prediction fails', async () => {
      ...
      await expect(runSAMSegmentation(params)).rejects.toThrow('Prediction failed');
    });
    ...
    it('should throw error when prediction is canceled', async () => {
      ...
      await expect(runSAMSegmentation(params)).rejects.toThrow('Prediction was canceled');
    });
    ...
    it('should throw error when no masks are returned', async () => {
      ...
      await expect(runSAMSegmentation(params)).rejects.toThrow('No masks returned');
    });
    ...
    it('should throw error when mask download fails', async () => {
      ...
      await expect(runSAMSegmentation(params)).rejects.toThrow('Failed to download mask');
    });
  });
