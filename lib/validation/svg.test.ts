...
describe('validateSVG', () => {
  ...
  describe('valid SVG formats', () => {
    ...
    it('should validate SVG starting with <!DOCTYPE svg', () => {
      const svg = '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd"><svg><rect /></svg>';
      const result = validateSVG(svg);
      expect(result.valid).toBe(true);
      expect(result.error).toBeUndefined();
    });
  ...
  describe('size validation', () => {
    ...
    it('should accept SVG at exactly MAX_SVG_SIZE', () => {
      ...
      const svgStart = '<svg>';
      const svgEnd = '</svg>';
      const contentSize = MAX_SVG_SIZE - Buffer.byteLength(svgStart + svgEnd, 'utf8');
  ...
  describe('dangerous tags detection', () => {
    ...
    it('should detect dangerous tags with attributes', () => {
      const svg = '<svg><script type="text/javascript">alert("xss")</script></svg>';
      const result = validateSVG(svg);
      expect(result.valid).toBe(false);
      expect(result.error).toContain('dangerous tag');
    });
  ...
  describe('width and height extraction', () => {
    ...
    it('should extract dimensions with other attributes present', () => {
      const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="200" viewBox="0 0 100 200"><rect /></svg>';
      const result = validateSVG(svg);
      expect(result.valid).toBe(true);
      expect(result.width).toBe(100);
      expect(result.height).toBe(200);
...
describe('sanitizeSVG', () => {
  describe('dangerous tags removal', () => {
    ...
    it('should remove multiple dangerous tags', () => {
      const svg = '<svg><script>bad1</script><iframe>bad2</iframe><rect /></svg>';
      const result = sanitizeSVG(svg);
      expect(result).not.toContain('<script');
      expect(result).not.toContain('<iframe');
  ...
  describe('javascript: protocol removal', () => {
    ...
    it('should remove all instances of javascript:', () => {
      const svg = '<svg><a href="javascript:x">A</a><use xlink:href="javascript:y" /></svg>';
      const result = sanitizeSVG(svg);
      expect(result).not.toContain('javascript:');
    });
  });
  ...
  describe('event handler removal', () => {
    ...
    it('should remove multiple event handlers', () => {
      const svg = '<svg onclick="fn1()" onload="fn2()" onmouseover="fn3()"><rect /></svg>';
      const result = sanitizeSVG(svg);
      expect(result).not.toContain('onclick');
      expect(result).not.toContain('onload');
      expect(result).not.toContain('onmouseover');
    ...
    it('should handle various event handler names', () => {
      const svg = '<svg onabort="x" onblur="x" onchange="x" onfocus="x"><rect /></svg>';
      const result = sanitizeSVG(svg);
  ...
  describe('data URI removal', () => {
    ...
    it('should remove data:image/svg+xml URIs', () => {
      const svg = '<svg><image href="data:image/svg+xml,<svg><script>alert(1)</script></svg>" /></svg>';
      const result = sanitizeSVG(svg);
  ...
  describe('safe content preservation', () => {
    ...
    it('should preserve safe attributes', () => {
      const svg = '<svg width="100" height="200" viewBox="0 0 100 200"><rect x="10" y="20" width="30" height="40" fill="blue" /></svg>';
      const result = sanitizeSVG(svg);
      expect(result).toBe(svg);
    });

    it('should preserve paths and shapes', () => {
      const svg = '<svg><path d="M 10 10 L 90 90" /><circle cx="50" cy="50" r="40" /></svg>';
      const result = sanitizeSVG(svg);
      expect(result).toBe(svg);
    });
    ...
    it('should preserve gradients and filters', () => {
      const svg = '<svg><defs><linearGradient id="g1"><stop offset="0%" /></linearGradient></defs></svg>';
      const result = sanitizeSVG(svg);
  ...
  describe('complex sanitization scenarios', () => {
    ...
    it('should remove dangerous attributes while preserving element', () => {
      const svg = '<svg><rect onclick="bad()" fill="blue" width="10" height="10" /></svg>';
      const result = sanitizeSVG(svg);
      expect(result).toContain('<rect');
