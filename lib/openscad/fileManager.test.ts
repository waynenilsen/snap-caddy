...
describe('STLFileManager', () => {
  ...
  beforeEach(async () => {
    ...
    testTempDir = join(tmpdir(), `snap-caddy-test-${Date.now()}-${randomBytes(3).toString('hex')}`);
  ...
  describe('createJobPaths', () => {
    ...
    it('creates the job directory on filesystem', async () => {
      ...
      const dirExists = await fs.stat(paths.jobDir).then(() => true).catch(() => false);
  ...
  describe('isFileExpired', () => {
    ...
    it('returns false for recently created file with large max age', async () => {
      ...
      const expired = await fileManager.isFileExpired(paths.svgPath, 1000 * 60 * 60 * 24); // 24 hours
  ...
  describe('integration tests', () => {
    ...
    it('handles multiple concurrent jobs', async () => {
      ...
      // Write to all jobs
      await Promise.all(
        paths.map((p, i) => fileManager.writeSVG(p.svgPath, `<svg id="${i}"></svg>`))
      );
