...
`;
      ...
      const result = generator.populateTemplate(template, '/path/to/file.svg', mockConfig);
    ...
    it('should handle multiple occurrences of the same placeholder', () => {
      ...
      const result = generator.populateTemplate(template, '/test.svg', mockConfig);
    ...
    it('should apply all padding values from cutoutPadding', () => {
      ...
      const template = '{{PADDING_TOP}} {{PADDING_BOTTOM}} {{PADDING_LEFT}} {{PADDING_RIGHT}}';
      const result = generator.populateTemplate(template, '/test.svg', configWithPadding);
    ...
    it('should use default corner radius when not specified', () => {
      ...
      const result = generator.populateTemplate(template, '/test.svg', configWithoutRadius);
  ...
  describe('baseTypeToNumber', () => {
    ...
    it('should map solid to 0', () => {
      ...
      const result = generator.populateTemplate(template, '/test.svg', { ...baseConfig, baseType: 'solid' });
      expect(result).toBe('0');
    });

    it('should map magnet to 1', () => {
      const template = '{{BASE_STYLE}}';
      const result = generator.populateTemplate(template, '/test.svg', { ...baseConfig, baseType: 'magnet' });
      expect(result).toBe('1');
    });

    it('should map screw to 2', () => {
      const template = '{{BASE_STYLE}}';
      const result = generator.populateTemplate(template, '/test.svg', { ...baseConfig, baseType: 'screw' });
      expect(result).toBe('2');
    });

    it('should map magnet_screw to 3', () => {
      const template = '{{BASE_STYLE}}';
      const result = generator.populateTemplate(template, '/test.svg', { ...baseConfig, baseType: 'magnet_screw' });
  ...
  describe('lipStyleToNumber', () => {
    ...
    it('should map normal to 0', () => {
      ...
      const result = generator.populateTemplate(template, '/test.svg', { ...baseConfig, lipStyle: 'normal' });
      expect(result).toBe('0');
    });

    it('should map reduced to 1', () => {
      const template = '{{LIP_STYLE}}';
      const result = generator.populateTemplate(template, '/test.svg', { ...baseConfig, lipStyle: 'reduced' });
      expect(result).toBe('1');
    });

    it('should map none to 2', () => {
      const template = '{{LIP_STYLE}}';
      const result = generator.populateTemplate(template, '/test.svg', { ...baseConfig, lipStyle: 'none' });
      expect(result).toBe('2');
    });
  });
  ...
  describe('escapeScadString', () => {
    ...
    it('should escape backslashes', () => {
      ...
      const result = generator.populateTemplate(template, 'C:\\path\\to\\file.svg', baseConfig);
      expect(result).toBe('C:\\\\path\\\\to\\\\file.svg');
    });

    it('should escape double quotes', () => {
      const template = '{{SVG_FILE_PATH}}';
      const result = generator.populateTemplate(template, '/path/with"quotes"/file.svg', baseConfig);
      expect(result).toBe('/path/with\\"quotes\\"/file.svg');
    });

    it('should escape both backslashes and quotes', () => {
      const template = '{{SVG_FILE_PATH}}';
      const result = generator.populateTemplate(template, 'C:\\path\\"quoted"\\file.svg', baseConfig);
      expect(result).toBe('C:\\\\path\\\\\\"quoted\\"\\\\file.svg');
    });

    it('should handle strings without special characters', () => {
      const template = '{{SVG_FILE_PATH}}';
      const result = generator.populateTemplate(template, '/simple/path/file.svg', baseConfig);
  ...
  describe('edge cases', () => {
    ...
    it('should handle decimal values correctly', () => {
      ...
      const template = '{{BIN_HEIGHT}} {{CUTOUT_DEPTH}} {{WALL_THICKNESS}} {{CORNER_RADIUS}}';
      const result = generator.populateTemplate(template, '/test.svg', config);
    ...
    it('should handle template with no placeholders', () => {
      ...
      const result = generator.populateTemplate(template, '/test.svg', baseConfig);
