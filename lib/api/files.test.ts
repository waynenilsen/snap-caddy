...
describe('FileManager', () => {
  ...
  describe('getTempFilePath', () => {
    ...
    it('should handle various filename formats', () => {
      ...
      expect(fileManager.getTempFilePath(fileId, 'data.json'))
        .toBe(join(MOCK_TEMP_DIR, fileId, 'data.json'));

      expect(fileManager.getTempFilePath(fileId, 'archive.tar.gz'))
        .toBe(join(MOCK_TEMP_DIR, fileId, 'archive.tar.gz'));

      expect(fileManager.getTempFilePath(fileId, 'noextension'))
        .toBe(join(MOCK_TEMP_DIR, fileId, 'noextension'));
    });
  });
  ...
  describe('scheduleCleanup', () => {
    ...
    it('should handle multiple file IDs independently', async () => {
      ...
      expect(fs.rm).toHaveBeenCalledWith(
        join(MOCK_TEMP_DIR, fileId1),
        { recursive: true, force: true }
      );
      expect(fs.rm).toHaveBeenCalledWith(
        join(MOCK_TEMP_DIR, fileId2),
        { recursive: true, force: true }
      );
    });
  });
  ...
  describe('cleanupOldFiles', () => {
    it('should clean up files older than maxAge', async () => {
      ...
      spyOn(fs, 'stat').mockImplementation(async (path: any) => {
        ...
        if (pathStr.includes('old-file-1') || pathStr.includes('old-file-2')) {
          return { mtimeMs: oldFileAge } as any;
        } else if (pathStr.includes('recent-file')) {
          return { mtimeMs: recentFileAge } as any;
        }
      ...
      expect(fs.rm).toHaveBeenCalledWith(
        join(MOCK_TEMP_DIR, 'old-file-1'),
        { recursive: true, force: true }
      );
      expect(fs.rm).toHaveBeenCalledWith(
        join(MOCK_TEMP_DIR, 'old-file-2'),
        { recursive: true, force: true }
      );
    ...
    it('should handle stat errors gracefully', async () => {
      ...
      spyOn(fs, 'stat').mockImplementation(async (path: any) => {
        ...
        return { mtimeMs: Date.now() - 7200000 } as any; // 2 hours old
      });
    ...
    it('should return 0 when no files need cleanup', async () => {
      ...
      spyOn(fs, 'readdir').mockResolvedValue(mockEntries as any);
      spyOn(fs, 'stat').mockResolvedValue({ mtimeMs: now - 1000 } as any); // 1 second old
  ...
  describe('shutdown', () => {
    it('should clear auto cleanup timer', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('File manager shutdown complete');
